...
async function cargarReservas(page = 1) {
    ...
    try {
        ...
        const response = await fetch(`https://backend-salones.vercel.app/api/reservas?page=${page}&limit=${limitePorPagina}&sort=ASC&fecha_inicio=${hoy}`, {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include' 
        });
        const data = await response.json();
        console.log('Datos recibidos:', data.reservas);
